...
import { Pencil, Eye, ChevronRight, Plus, X } from 'lucide-react';
...
function OptionsList({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {options.map((opt, i) => (
            <div key={i} style={{
              ...
              <span style={{
                ...
                background: '#FFF1F2', border: '1.5px solid rgba(225,29,72,0.25)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: '#374151', fontFamily: 'system-ui' }}>
                {opt}
              </span>
              <button
      ...
      <p style={{ fontSize: 11, color: '#9CA3AF', margin: 0, fontFamily: 'system-ui' }}>
...
export default function RoletaUpsellPage() {
  ...
  const [previewProduct, setPreviewProduct] = useState<ProductKey>('roulette');
  ...
  const handleAdd = () => {
    setAdded(true);
    setPreviewProduct('roulette');
    saveWithRoulette(title, options.length > 0 ? options : DEMO_ROULETTE.options, true);
    if (options.length === 0) {
      setOptions(DEMO_ROULETTE.options);
    }
  };
  const handleSkip = () => {
    saveWithRoulette(title, options, false);
    router.push('/criar/upsell');
  };
  const handleContinue = () => {
    saveWithRoulette(title, options, true);
    router.push('/criar/upsell');
  };
  const back = () => {
  ...
  const ok = !added || options.length >= 2;
          ...
          <div style={{ overflowY: 'auto', padding: '32px 0 24px', display: 'flex', justifyContent: 'center' }}>
            <div style={{ width: '100%', maxWidth: 520, padding: '0 24px' }}>
              ...
              <div className="hidden lg:block">
                <Stepper current={7} total={8} onGoto={onGoto} />
                ...
                {!added ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '24px 0' }}>
                    ...
                    <h2 style={{
                      ...
                      O presente que não termina no link.
                    </h2>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <StepHeader
                      ...
                      description="Crie uma roleta com atividades para fazerem juntos."
                    />
                    ...
                    <div style={{
                      ...
                      <p style={{ fontSize: 13, fontWeight: 700, color: '#E11D48', margin: 0, fontFamily: 'system-ui' }}>
                        Roleta adicionada! Adicione as opções da roleta:
                      </p>
                    </div>

                    <FieldCard icon="🎡" label="Título da roleta" focused={focused === 'rtitle'}>
                      <input
                        style={inlineInput}
                        type="text"
                        value={title}
                        ...
                        maxLength={50}
                        onFocus={() => setFocused('rtitle')}
                        onBlur={() => setFocused(null)}
                        onChange={e => {
                          setTitle(e.target.value);
                          saveWithRoulette(e.target.value, options, true);
                        }}
                      />
                    </FieldCard>

                    <div style={{ margin: '12px 0' }}>
                      <span style={{ display: 'block', fontSize: 11, fontWeight: 800, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8, fontFamily: 'system-ui' }}>
                        Opções da Roleta
                      </span>
                      <OptionsList
                        options={options}
                        onChange={opts => {
                          setOptions(opts);
                          saveWithRoulette(title, opts, true);
                        }}
                      />
                    </div>
        ...
        >
          {mobileView === 'preview' && (
            <LivePreview
              ...
              extras={added ? [...funnel.extras.filter(e => e !== 'roulette'), 'roulette'] : funnel.extras.filter(e => e !== 'roulette')}
              wordle={funnel.wordle}
              roulette={previewData}
          ...
          <div style={{
            ...
            <div>
              ...
              <span style={{
                ...
                {added ? 'Roleta Surpresa' : 'Preview do Presente'}
              </span>
            </div>
            <div style={{
